<x-layouts.frasear :title="config('app.name', 'Fraseario') . ' — a collection of phrases worth keeping'">
    <div
            x-data="phraseSlider({{ $phrases->map(fn ($p) => [
            'body' => $p->body,
            'author' => $p->author?->name,
            'saved_by' => $p->savedBy->username,
            'saved_by_url' => route('frasear.show', $p->savedBy->username),
        ])->toJson() }})"
    ...
    >
        ...
        <header class="shrink-0 border-b border-line px-5 py-4 sm:px-8">
            <div class="mx-auto flex max-w-5xl items-center justify-between gap-6">
                ...
                <form action="{{ route('phrases.search') }}" method="GET" class="w-full max-w-xs">
                    <label for="tag-search" class="sr-only">Search by tag</label>
                    <input
                            id="tag-search"
                            type="search"
                            name="tag"
        ...
        {{-- Slider frame: swipe only, no vertical scroll --}}
        <main
        ...
        >
            ...
            @else
                <template x-for="(phrase, i) in phrases" :key="i">
                    ...
                    >
                        <blockquote class="max-w-[62ch] text-center">
                            <p class="font-serif text-[1.75rem] italic leading-snug text-ink sm:text-[2.5rem]" x-text="'“' + phrase.body + '”'"></p>
                        </blockquote>

                        <div class="pointer-events-none absolute inset-x-6 bottom-6 flex items-end justify-between sm:inset-x-10 sm:bottom-10">
                            <p class="pointer-events-auto font-serif text-sm text-muted" x-show="phrase.author" x-text="'— ' + phrase.author"></p>
                ...
                {{-- Position dots — a real sequence indicator, not decoration --}}
